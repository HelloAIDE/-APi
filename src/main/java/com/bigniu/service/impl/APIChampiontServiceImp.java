...
import com.lol.empty.Info;
import com.lol.empty.Skin;
import com.lol.empty.Spell;
import com.lol.util.GetNetDataUtil;
import com.lol.util.ParseJsonUtil;
import com.lol.util.URLCodeUtil;
@Service
public class APIChampiontServiceImp implements APIChampiontService {
	...
	public List<Area> getAllArea() {
		...
		String data = GetNetDataUtil.GetNetData(Static.AreaURL, null, false);
		String jsonData = data.substring(data.indexOf("map=") + 4, data.indexOf(";if(typeof"));
		JsonNode rootNode = ParseJsonUtil.getJsonNode(jsonData);
		Iterator<String> iterator = rootNode.fieldNames();
		for (int i = 0; i < rootNode.size(); i++) {
			JsonNode spellNode = rootNode.get(i);
			Area area = new Area();
			area.setId(spellNode.findValue("id").asText());
			area.setIdc(spellNode.findValue("idc").asText());
			area.setIsp(spellNode.findValue("isp").asText());
			area.setName(spellNode.findValue("name").asText());
			area.setOb(spellNode.findValue("ob").asText());
			area.setStrid(spellNode.findValue("strid").asText());
			area.setTcls(spellNode.findValue("tcls").asText());
			list.add(area);
		}
	...
	public List<Champion> getFreeChampion() {
		List<Champion> champions = getAllChampion();
